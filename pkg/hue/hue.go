...
func toggleLight(light int, url string, state bool) {
	...
	url = fmt.Sprint("%slights/%d/state", url, light)
...
func FlickMeDaddy(client *twitch.Client, lights []int) {
...
—————————————————————————————`)
	...
	for _, light := range lights {
		toggleLight(light, url, true)
		duration := time.Duration(3) * time.Second
		time.Sleep(duration)
		toggleLight(light, url, false)
	}
}
